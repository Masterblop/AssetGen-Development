...
class GA_PT_generatePanel(bpy.types.Panel):
    ...
    bl_category = "AssetGen-Development"
    ...
    def draw(self, context):
        ...
        if myscene.ga_TRIANGULATE == False:
	        row = col_ga.row()
	        row.prop(myscene , "ga_UNSUBDIV")
	        col_ga.row().separator()

        if myscene.ga_TRIANGULATE == False:
	        row = col_ga.row()
	        row.prop(myscene , "ga_iterations")
	        col_ga.row().separator()
	        row = col_ga.row()

        row = col_ga.row()
        row.prop(myscene , "ga_TRIANGULATE")
        col_ga.row().separator()
