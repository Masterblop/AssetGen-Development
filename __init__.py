...
class GA_Props(bpy.types.PropertyGroup):
   ...
   ga_UNSUBDIV : bpy.props.BoolProperty(
        name = 'Un-Subdivide',
        description = "Reduce the polycount by unsubdividing the model",
        default = False
   )

   ga_iterations : bpy.props.IntProperty(
        name = 'Un-Subdivide iterations',
        description = "Suggested multiple numbers of 2",
        default = 2,
        min = 1,
        max = 10
   )

   ga_TRIANGULATE : bpy.props.BoolProperty(
        name = 'Triangulate',
        description = "Triangulate your model during the process",
        default = True
   )
