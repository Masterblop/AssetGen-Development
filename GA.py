...
class GA_Start(bpy.types.Operator):
	...
	"""Will generate your game asset, Blender will freeze for few seconds.
	Follow the process in Window > System Console.
	IMPORTANT: the diffuse map can only be baked from an Emissive shader"""
	...
	def execute(self, context):
		...
		un_subdivide = myscene.ga_UNSUBDIV
		iterations = myscene.ga_iterations
...
######################################################### Selected to active = 0
		...
		if selected_to_active == 0:
			...
			if myscene.ga_ontheground == 1:
				lowest_pt = min([(bpy.context.object.matrix_world @ v.co).z for
...
######################################################### Remove underground = 1
		...
		if rmv_underground == 1:
			...
			bpy.ops.mesh.bisect(
				plane_co=(0.00102639, 0.0334111, 0),
				plane_no=(0, 0, 0.999663),
				use_fill=False,
				clear_inner=True,
				xstart=295,
				xend=444,
...
################################################################### Decimation 1
			...
			if un_subdivide == 0:
				...
				bpy.context.object.modifiers[
					"Decimate"].ratio = LOD0 / mesh_polycount
			...
			if un_subdivide == 1:
				...
				bpy.context.object.modifiers[
					"Decimate"].iterations = bpy.context.scene.ga_property.ga_iterations
...
############## Remove the previous LODs with the same names to update the result
...
################################################################ EXPORT THE MESH
		...
		if myscene.ga_file == "glb":
			bpy.ops.export_scene.gltf(
				...
				filepath=os.path.join(path, "SM_" + name))
		...
		print("LOD0:",
			len(bpy.context.active_object.data.polygons), "tris")
...
########################################################################### LOD1
		...
		if LOD1 > 0:
			...
			bpy.context.object.modifiers[
				"Decimate"].ratio = LOD1 / mesh_polycount
...
########################################################################### LOD2
		...
		if LOD2 > 0:
			...
			bpy.context.object.modifiers[
				"Decimate"].ratio = LOD2 / mesh_polycount
...
########################################################################### LOD3
		...
		if LOD3 > 0:
			...
			bpy.context.object.modifiers[
				"Decimate"].ratio = LOD3 / mesh_polycount
			...
			print("LOD2:", len(
				bpy.context.active_object.data.polygons), "tris")
...
###################################################   FBX not aviable now ||###
		...
		# Moving the LODs behind the high poly
...
#################################If Show Output is set to false, delete the LODs
		...
		if myscene.ga_showoutput == 0:
			...
			bpy.ops.object.select_pattern(
				pattern= "SM_" + name + "_LOD0")

			bpy.ops.object.delete(use_global=False)

			bpy.ops.object.select_pattern(
				pattern= "SM_" + name + "_LOD1")

			bpy.ops.object.delete(use_global=False)

			bpy.ops.object.select_pattern(
				pattern= "SM_" + name + "_LOD2")

			bpy.ops.object.delete(use_global=False)

			bpy.ops.object.select_pattern(
				pattern= "SM_" + name + "_LOD3")
